#!/usr/bin/env python3
"""Allegro Lokalnie → Scout bridge: normalizer from Allegro API JSONL / HTML text dumps to scout schema JSONL.

Reads Allegro Lokalnie data from two possible sources:
1. JSONL export (from Allegro API / web scraper)
2. HTML-lines text dump (copy-paste from Allegro Lokalnie pages, heuristic parsing)
...
Usage:
python3 scout_ingest_allegro_lokalnie.py --jsonl <allegro_export.jsonl> --output <scout_incoming.jsonl>
python3 scout_ingest_allegro_lokalnie.py --html-lines <allegro_html_dump.txt> --output <scout_incoming.jsonl>
python3 scout_ingest_allegro_lokalnie.py --auto --source-dir <scout_data/allegro_lokalnie/> --output <scout_incoming.jsonl>
"""
...
def _extract_price(raw: dict[str, Any]) -> tuple[int | None, str, str]:
    ...
    if price_value is not None:
        try:
            pv = int(price_value)
            label = raw.get("price_label") or f"{pv} zł"
            return pv, "PLN", label
        except (ValueError, TypeError):
            pass

    label = raw.get("price_label") or ""
    if label:
...
def normalize_allegro_jsonl_record(raw: dict[str, Any], seq: int = 0) -> dict[str, Any]:
    raw_id = raw.get("id", "")
    if not raw_id:
        raw_id = f"al-{hashlib.md5((raw.get('title','') + raw.get('description','')).encode()).hexdigest()[:12]}"

    if not raw_id.startswith("al-"):
        raw_id = f"al-{raw_id}"
    ...
    return {
        ...
        "source_detail": raw.get("category", ""),
...
def parse_html_lines_dump(text_path: Path) -> list[dict[str, Any]]:
    ...
    for block in blocks:
        ...
        header_match = re.match(r"\[Ogloszenie\s+(\d+)\]", lines[0])
        if header_match:
            lines = lines[1:]
            if not lines:
                continue
        ...
        price_value = None
        price_label = ""
        price_match = re.search(r"(\d+)\s*z[łl]", title_line + " " + description)
        if price_match:
            price_value = int(price_match.group(1))
            price_label = f"{price_value} zł"

        is_free = any(kw in title_line.lower() or kw in description.lower() for kw in FREE_KEYWORDS)
        ...
        full_text = (title_line + " " + description).lower()
        ...
        offer_id = f"al-txt-{hashlib.md5(title_line.encode()).hexdigest()[:8]}"
...
def ingest_jsonl(jsonl_path: Path, output: Path, *, filter_non_resource: bool = True) -> dict[str, Any]:
    ...
    normalized = [normalize_allegro_jsonl_record(r, i) for i, r in enumerate(raw_records)]
...
def ingest_html_lines(text_path: Path, output: Path, *, filter_non_resource: bool = True) -> dict[str, Any]:
    ...
    print(f" Ofert w text dump: {len(raw_offers)}")
...
def ingest_auto(source_dir: Path, output: Path, *, filter_non_resource: bool = True) -> dict[str, Any]:
    print(f"[scout_ingest_allegro_lokalnie] Auto-detect sources in: {source_dir}")
    ...
    for jsonl_path in jsonl_files:
        ...
        normalized = [normalize_allegro_jsonl_record(r, i) for i, r in enumerate(raw)]
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Allegro Lokalnie → Scout bridge: normalize Allegro JSONL/HTML-lines to scout schema JSONL",
    )
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("--jsonl", type=Path, help="Path to Allegro Lokalnie JSONL export")
    group.add_argument("--html-lines", type=Path, help="Path to Allegro Lokalnie HTML-lines text dump")
    ...
    receipt_path = Path(args.output).parent / f"ingest_receipt_al_59_{TODAY_STR}.json"
